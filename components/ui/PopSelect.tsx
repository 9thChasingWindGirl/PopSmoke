...
export const PopSelect: React.FC<PopSelectProps> = memo(({
  ...
  const content = (
    ...
    >
      <div className={`bg-white border-4 border-black p-6 w-full mx-4 ${isFirstLaunch ? 'max-w-md' : 'max-w-sm'}`}>
        <h3 className={`font-display text-2xl mb-4 border-b-2 border-black pb-2 ${getTitleColor()}`}>
          {title}
        </h3>
        {message && (
          <div className={`mb-4 text-gray-700 ${isFirstLaunch ? 'bg-yellow-50 border-2 border-yellow-400 p-3 rounded' : ''}`}>
            <p className={isFirstLaunch ? 'text-sm leading-relaxed' : 'text-sm'}>{message}</p>
          </div>
        )}
        <div className="space-y-3">
          {options.map((option) => (
            <button
              ...
              className={`relative w-full px-3 py-2 md:px-4 md:py-3 lg:px-6 lg:py-3 font-display text-sm md:text-base lg:text-xl uppercase tracking-wider border-4 border-black shadow-pop transition-all transform hover:shadow-pop-hover hover:translate-x-[2px] hover:translate-y-[2px] text-black ${isFirstLaunch ? 'py-4' : ''}`}
